...
trait Foldable[F[_]] {
  ...
  def foldMap[A, B: Monoid: Thunkable](fa: F[A])(f: A => B): B

  def foldRight[A, B: Thunkable](fa: F[A], z: B)(f: (A, B) => B): B

  def foldLeft[A, B: Thunkable](fa: F[A], z: B)(f: (B, A) => B): B // = TODO default implementation from foldmap
