...
sealed abstract class Node[V, A](implicit r: Reducer[A, V]) {
  ...
  def map[B, V2](f: A => B)(implicit m: Reducer[B, V2]): Node[V2, B] {
    def fold[B](two: (V2, B, B) => B, three: (V2, B, B, B) => B): B
...
object FingerTree extends FingerTreeInstances {
  ...
  def one[V, A](a: A)(implicit measure: Reducer[A, V]): One[V, A] =
    One(measure.unit(a), a)

  def two[V, A](a1: A, a2: A)(implicit measure: Reducer[A, V]): Two[V, A] =
    Two(measure.snoc(measure.unit(a1), a2), a1, a2)

  def three[V, A](a1: A, a2: A, a3: A)(implicit measure: Reducer[A, V]): Three[V, A] =
    Three(measure.snoc(measure.snoc(measure.unit(a1), a2), a3), a1, a2, a3)

  def four[V, A](a1: A, a2: A, a3: A, a4: A)(implicit measure: Reducer[A, V]): Four[V, A] =
