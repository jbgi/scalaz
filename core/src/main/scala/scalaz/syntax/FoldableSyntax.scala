...
final class FoldableOps[F[_],A] private[syntax](val self: F[A])(implicit val F: Foldable[F]) extends Ops[F[A]] {
  ...
  final def foldRightByName[B](z: => B)(f: (A, => B) => B): B = F.foldRightByName(self, z)(f)
  final def foldRight[B](z: B)(f: (A, B) => B): B = F.foldRight(self, z)(f)
  final def foldRightSuspend[B: Suspendable](z: => B)(f: (A, B) => B): B = F.foldRightSuspend(self, z)(f)
  ...
  final def foldrByName[B](z: => B)(f: A => (=> B) => B): B = F.foldrByName(self, z)(f)
  final def foldr[B](z: B)(f: A => B => B): B = F.foldr(self, z)(f)
  final def foldrSuspend[B: Suspendable](z: => B)(f: A => B => B): B = F.foldrSuspend(self, z)(f)
